...
#   Location generation of users
    ...
    # uniformly sample the parallelogram defined by the columns of
...
def crandn(shape,set_mag=None):
    ...
    #Y=np.random.normal(size=tuple(shape)+(2,))
    #X=Y.view(np.complex128)[:,0]
    ...
    def genX(self,batches=1):
        ...
        for i in range(batches):
            for c in range(C): #c==0 indicates all users are in the base stations' cell ("near")
                ...
                # Compute the large-scale fading coefficients
                # how far (weak) is each user?
                if c==0:
                    dist = abs( hexagonal_uniform( K ,as_complex=True) )
                elif 0<c<7:
                    dist = abs( 2+hexagonal_uniform( K ,as_complex=True) )  # symmetric topology of the hexagon cell
                else:
                    assert False,'assuming 1 or 7 hexagonal cells'
                dist.shape = (K,1,1)
                lsf_alpha = 15.3
                lsf_beta  = 37.6
                lsf = np.zeros(K).astype(np.float64)
                for il in range(K):
                    lsf[il] = 10.0**(-(lsf_alpha+lsf_beta*math.log10(dist[il]))/20.0)
                lsf.shape = (K,1,1)
                gain = 1/(1+dist**ple)

                # Compute the small-scale fading coefficients
                ssf = np.random.normal(size=tuple((K,Nr,1)))
                lsf_vec = 1.0 * lsf
                lsf_vec.shape = (K)
                ssf_mat = 1.0 * ssf
                ssf_mat.shape = (K,Nr)
                chl = np.dot(np.diag(lsf_vec), ssf_mat)
                chl.shape = (K,Nr,1)
